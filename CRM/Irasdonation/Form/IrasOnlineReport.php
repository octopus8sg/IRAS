  ...
  public function postProcess()
  {
    ...
    while ($result->fetch()) {
      ...
      if ($idType > 0) {
        $dataBody = array(
          ...
          'postalCode' => '',
          'donationAmount' => round($result->receipt_amount),
          'dateOfDonation' => date("Ymd", strtotime($result->issued_on)),
    ...
    if ($response != null) {
      ...
      foreach ($saveReport as $value) {
        $insert = "INSERT INTO civicrm_o8_iras_donation(cdntaxreceipts_log_id, is_api, log_id, created_date) VALUES ($value, 1, $log_id, '$genDate');";
        CRM_Core_DAO::executeQuery($insert, CRM_Core_DAO::$_nullArray);
      }
